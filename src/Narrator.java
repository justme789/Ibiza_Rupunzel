...
public class Narrator {
    int attitude = 0;
    String[] questions = { "what", "who", "when", "how", "why", "does", "has", "can", "shall", "is", "which", "did",
            "where" };
    ...
    public Pair<String, Integer> think(String s) {
        ...
        if (words.length >= 1) {
            ...
            for (int i = 0; i < greetings.length; i++) {
                if (words.length > 1
                        && ((words[0].toLowerCase().concat(words[1])).equals(greetings[i].toLowerCase()))) {
            ...
            return new Pair<>("Dont be silly", 0);
    }
